...
import { X, Trash2, ArrowUp, ArrowDown, AlignVerticalJustifyCenter } from 'lucide-react';
  ...
  const renderHighlightedText = (text: string, vocab: any[]) => {
    ...
    return parts.map((part, i) => {
      ...
      if (v) {
        ...
        // English -> Thinner/Denser Dashed Underline (border-b) -> No Bold -> Chinese (manual offset)
        return (
          <span key={i} className="group relative inline-block mx-[2px] align-baseline">
             ...
             {/* Chinese Meaning: Dynamic positioning */}
            <span 
              className="absolute left-1/2 -translate-x-1/2 top-full text-[0.45em] text-slate-700 font-sans font-normal whitespace-nowrap leading-none pointer-events-none transition-all duration-200"
    ...
    <>
      ...
      <div className="fixed bottom-32 right-8 z-[100] print:hidden flex flex-col gap-1 bg-white p-2 rounded-xl shadow-[0_4px_20px_rgba(0,0,0,0.15)] border border-gray-100 animate-in slide-in-from-right-10">
        <div className="text-[10px] text-center text-gray-400 font-bold mb-1 uppercase tracking-wider flex justify-center items-center gap-1">
          <AlignVerticalJustifyCenter size={12}/> CN Pos
        </div>
        <button 
